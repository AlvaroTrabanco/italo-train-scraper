...
def main() -> None:
    ...
    def ensure_stop(code: str, name: str) -> str:
        # Use stable stop_id based on code if present; else fallback to name.
        key = code or name
    ...
    for f in sorted(norm_files):
        ...
        # route_short_name can be train number
        routes_rows.append([route_id, agency_id, str(train), f"{origin} – {dest}", "2"])

        # trips.txt (one trip per train per service date)
        ...
        for s in data["stops"]:
            ...
            code = s.get("location_code") or ""
            stop_id = ensure_stop(code, name)

            arr = s.get("arrival_time") or ""
            dep = s.get("departure_time") or ""
            # GTFS requires both for intermediate stops; many feeds still leave first arrival / last departure blank.
            stop_times_rows.append([trip_id, arr, dep, stop_id, str(seq)])
            seq += 1
